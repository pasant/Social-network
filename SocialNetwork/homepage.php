...
<div class="profilePosts">
	...
	$sql1="SELECT * FROM posts inner join users on posts.user_id=users.id
	left join friends  on posts.user_id= friends.friend_id='$id'
	 ";
	 
	$getposts = mysqli_query($connect,$sql1) or die(mysqli_error($connect)); 
    while($record = mysqli_fetch_array($getposts))
	{
		if($record['friend_id'] != null || $record['isPublic']=='public' || $record['user_id']='$id')
		{
